...
برای دریافت لینک اشتراک، QR و کانفیگ‌ها از دکمه‌های زیر استفاده کنید.`, { reply_markup: { inline_keyboard: [[{ text: "📦 مشاهده سرویس", callback_data: callbackFor("account.xray", { xrayClientId: client.id }) }], [{ text: "🔗 دریافت لینک اشتراک", callback_data: `xray:sub:${client.id}` }, { text: "⚙️ دریافت کانفیگ‌ها", callback_data: `xray:configs:${client.id}` }], [{ text: "🏠 خانه", callback_data: callbackFor("home") }]] } });
  ...
  bot.action(/^xray:renew:wallet:([^:]+):([^:]+)$/, async (ctx) => {
    ...
    try {
      ...
      const renewal = await PaymentInvoiceService.renewXrayWithWallet(user.id, ctx.match[1], ctx.match[2]);
      await ctx.reply(`✅ سرویس با موفقیت تمدید شد.\n\nاعتبار جدید: ${renewal.newExpiry.toLocaleDateString("fa-IR")}`, { reply_markup: { inline_keyboard: [[{ text: "🧩 مشاهده سرویس", callback_data: callbackFor("account.xray", { xrayClientId: ctx.match[1] }) }]] } });
  ...
  bot.action(/^xray:renew:instant:([^:]+):([^:]+)$/, async (ctx) => {
    ...
    if (!user) return;
    try {
      const invoice = await PaymentInvoiceService.createXrayRenewalInvoice(user.id, ctx.match[1], ctx.match[2]);
      await ctx.reply(`🧾 فاکتور تمدید آماده شد\n\n💰 مبلغ: ${invoice.amount.toLocaleString("fa-IR")} تومان\n\nبرای پرداخت روی دکمه زیر بزنید.`, { reply_markup: { inline_keyboard: [[{ text: "⚡ پرداخت", url: invoice.paymentLink ?? "" }], [{ text: "🔙 بازگشت", callback_data: callbackFor("account.xray", { xrayClientId: ctx.match[1] }) }]] } });
  ...
  async function completeProductCreateFromPicker(ctx: AppContext) {
    ...
    await ProductService.create({ categoryId, title: String(flow.data.title), price: Number(flow.data.price), duration: Number(flow.data.duration), trafficGB: Number(flow.data.trafficGB), stockLimit: Number(flow.data.stockLimit), inboundIds: flow.data.inboundIds as unknown as number[], inboundSnapshot: String(flow.data.inboundSnapshot), limitIp: Number(flow.data.limitIp ?? 0), xrayGroupName: flow.data.xrayGroupName ? String(flow.data.xrayGroupName) : null });
    ctx.session.flow = undefined;
  }
