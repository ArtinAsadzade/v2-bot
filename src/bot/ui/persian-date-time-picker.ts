...
import { BOT_TIME_ZONE, formatJalaliDateTime, getSelectableJalaliYears, jalaliMonthLength, PERSIAN_MONTH_NAMES, zonedJalaliToUtcDate } from "../../utils/persianDateTime";
...
const isValidPickerButton = (button: { text?: string; action?: string }) => Boolean(button.text?.trim()) && !/NaN|undefined|null/.test(button.text ?? "") && Boolean(button.action?.trim());
const row = <T>(items: T[], size: number) => Array.from({ length: Math.ceil(items.length / size) }, (_, i) => items.slice(i * size, i * size + size));
const safeRows = (keyboard: UiKeyboard): UiKeyboard => keyboard.map((buttons) => buttons.filter(isValidPickerButton)).filter((buttons) => buttons.length > 0);
export const dtpAction = (...parts: (string | number)[]) => actionFor("dtp", ...parts);
export function startPersianDateTimePicker(ctx: AppContext, state: DateTimePickerState) { ctx.session.dateTimePicker = { ...state }; }
...
export function pickerKeyboard(state: DateTimePickerState, step: string): UiKeyboard {
  if (step === "year") {
    ...
    return safeRows([...row(yearButtons, 2), [{ text: "❌ لغو", action: dtpAction("cancel"), tone: "danger" as const }]]);
  }
  if (step === "month") return safeRows([...row(PERSIAN_MONTH_NAMES.map((m,i)=>({ text:m, action:dtpAction("m", i+1), tone:"primary" as const })),2), [{ text:"🔙 تغییر سال", action:dtpAction("back","year"), tone:"neutral" as const }, { text:"❌ لغو", action:dtpAction("cancel"), tone:"danger" as const }]]);
  if (step === "day") { const days = Array.from({length: jalaliMonthLength(state.selectedYear!, state.selectedMonth!)},(_,i)=>i+1); return safeRows([...row(days.map(d=>({text:fa(d), action:dtpAction("d",d), tone:"primary" as const})),7), [{text:"🔙 تغییر ماه", action:dtpAction("back","month"), tone:"neutral" as const},{text:"❌ لغو", action:dtpAction("cancel"), tone:"danger" as const}]]); }
  if (step === "hour") return safeRows([...row(Array.from({length:24},(_,i)=>({text:pad(i), action:dtpAction("h",i), tone:"primary" as const})),4), [{text:"🔙 تغییر روز", action:dtpAction("back","day"), tone:"neutral" as const},{text:"❌ لغو", action:dtpAction("cancel"), tone:"danger" as const}]]);
  if (step === "minute") return safeRows([...row(Array.from({length:12},(_,i)=>i*5).map(m=>({text:pad(m), action:dtpAction("min",m), tone:"primary" as const})),4), [{text:"🔙 تغییر ساعت", action:dtpAction("back","hour"), tone:"neutral" as const},{text:"❌ لغو", action:dtpAction("cancel"), tone:"danger" as const}]]);
  return safeRows([[{text:"✅ تأیید زمان", action:dtpAction("confirm"), tone:"success" as const}], [{text:"🔙 تغییر", action:dtpAction("back","year"), tone:"neutral" as const},{text:"❌ لغو", action:dtpAction("cancel"), tone:"danger" as const}]]);
}
