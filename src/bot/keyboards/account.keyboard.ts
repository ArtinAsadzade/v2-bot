import { callbackFor, panelKeyboard, type UiKeyboard } from "../navigation/panel-ui";
...
export const xrayConfigsSentKeyboard = (clientId: string) => ({
  inline_keyboard: [
    [
      { text: "🔗 لینک اشتراک", callback_data: xrayCallbacks.subscription(clientId) },
      { text: "🔙 بازگشت", callback_data: callbackFor("account.xray", { xrayClientId: clientId }) },
    ],
  ],
});
...
export function accountActionViewKeyboard(xrayClientId: string, options: { renewable?: boolean } = {}): UiKeyboard {
  return [
    [
      {
        text: accountActionLabels.subscription,
        action: xrayCallbacks.subscription(xrayClientId),
      },
      {
        text: "📲 دریافت QR اشتراک",
        action: xrayCallbacks.qr(xrayClientId),
      },
    ],
    [
      {
        text: accountActionLabels.configs,
        action: xrayCallbacks.configs(xrayClientId),
      },
    ],
    options.renewable
      ? [
          {
            text: accountActionLabels.refresh,
            action: callbackFor("account.xray", { xrayClientId }),
          },
          {
            text: accountActionLabels.renew,
            action: callbackFor("account.renew", { xrayClientId }),
          },
        ]
      : [
          {
...
export function accountActionKeyboard(xrayClientId: string) {
  return panelKeyboard(accountActionViewKeyboard(xrayClientId, { renewable: true }), {
    back: false,
    home: false,
  });
}
