...
export function registerWalletViews() {
  registerView("wallet", async (ctx) => {
    ...
    return {
      ...
      text: joinSections([
        card(userLabels.wallet, [`موجودی فعلی: ${money(user?.balance ?? 0)}`]),
        section(`${uiIcons.invoice} تراکنش‌های اخیر`, [recent]),
  ...
  const renderWalletHistory = async (ctx: Parameters<import("../navigation/panel-ui").ViewRenderer>[0]) => {
    ...
    return {
      text: joinSections([
        card(`${uiIcons.invoice} گردش کیف پول`, [
          dashboard.walletTransactions
            .map(
              (tx) =>
                `${tx.type === "credit" || tx.type === "transfer_in" ? "🟢" : "🔴"} ${tx.description}\n${money(tx.amount)} · ${tx.createdAt.toLocaleString("fa-IR")}`,
            )
  ...
  registerView("wallet.balance", async (ctx) => {
    const user = ctx.from ? await UserService.getByTelegramId(ctx.from.id) : undefined;
    return {
      text: card("💳 موجودی", [`موجودی فعلی: ${money(user?.balance ?? 0)}`]),
      keyboard: [navRow({ text: "➕ افزایش موجودی", view: "wallet.topup", tone: "success" })],
    };
  });
  registerView("wallet.transactions", async (ctx) => renderWalletHistory(ctx));
  registerView("wallet.invoices", async () => ({
    text: card("🧾 فاکتورها", ["فاکتورهای پرداخت از بخش خرید و پرداخت قابل پیگیری هستند."]),
    keyboard: [],
  }));
  ...
  registerView("deposit", async () => ({
    text: "در حال انتقال به افزایش موجودی...",
    keyboard: [navRow({ text: "➕ افزایش موجودی", view: "wallet.topup", tone: "success" })],
  }));
}
