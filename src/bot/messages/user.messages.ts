...
export function userHomeMessage(input: { firstName?: string; balance: string; activeServices: number }) {
  return joinSections([
    ...
    section(sectionTitles.quickActions, [`💳 موجودی کیف پول: ${input.balance}`, `📦 سرویس‌های فعال: ${input.activeServices.toLocaleString("fa-IR")}`, "یکی از گزینه‌های زیر را انتخاب کنید."]),
  ]);
}
