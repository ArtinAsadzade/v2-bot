...
export type MessageTone = keyof Pick<
  typeof UI_EMOJI,
  "SUCCESS" | "ERROR" | "WARNING" | "PAYMENT" | "WALLET" | "SUPPORT" | "USER" | "ADMIN" | "STATS" | "SETTINGS" | "PRODUCT"
>;
...
export function successMessage(
  title: string,
  description = "درخواست شما با موفقیت انجام شد.",
...
export function errorMessage(
  title = "انجام درخواست ممکن نیست",
  description = "در حال حاضر امکان انجام این درخواست وجود ندارد.",
  actionHint = "لطفاً دوباره تلاش کنید یا با پشتیبانی در ارتباط باشید.",
) {
...
export function paymentSummaryMessage(data: {
  productTitle?: string;
  amount: number;
  ...
  return screenMessage({
    ...
    body: [
      ...
      data.shortage
        ? `${UI_EMOJI.WARNING} کسری موجودی: ${money(data.shortage)}`
        : data.balance !== undefined
          ? `${UI_EMOJI.SUCCESS} موجودی شما برای خرید کافی است.`
          : undefined,
      "",
...
export function purchaseSuccessMessage(data: {
  productTitle: string;
...
export function accountSummaryMessage(data: {
  balance: number;
