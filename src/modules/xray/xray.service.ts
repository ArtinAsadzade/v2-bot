...
export class XrayClientService {
 ...
 static async listGroups(config?: { apiBaseUrl: string; apiToken: string }) { const res = await request<{ obj?: XrayClientGroup[] }>("/panel/api/clients/groups", {}, config); const groups = (res.obj ?? []).filter((group) => typeof group.name === "string" && group.name.trim()).map((group) => ({ ...group, name: group.name.trim() })); logger.info("XRAY_GROUPS_FETCHED", { count: groups.length }); return groups; }
 static async createClient(input: { email: string; trafficBytes: bigint; expiresAt: Date; telegramId: string | number; inboundIds: number[]; limitIp?: number; groupName?: string | null }) { logger.info("XRAY_CLIENT_CREATE_REQUEST", { email: input.email, inboundIds: input.inboundIds, limitIp: input.limitIp ?? 0, groupName: input.groupName }); const client: Record<string, unknown> = { email: input.email, totalGB: Number(input.trafficBytes), expiryTime: input.expiresAt.getTime(), tgId: Number(input.telegramId), limitIp: Math.max(0, Number(input.limitIp ?? 0)), enable: true }; if (input.groupName) client.group = input.groupName; const res = await request<{ obj?: { id?: string; uuid?: string; subId?: string } }>("/panel/api/clients/add", { method: "POST", body: JSON.stringify({ client, inboundIds: input.inboundIds }) }); logger.info("XRAY_CLIENT_CREATED", { email: input.email }); return res.obj ?? {}; }
 static async updateClient(email: string, input: { totalBytes: bigint; expiresAt: Date; telegramId: string | number; limitIp?: number; groupName?: string | null }) { logger.info("XRAY_CLIENT_RENEW_REQUEST", { email }); try { const existing = await this.getClient(email).catch(() => null); const existingClient = existing?.obj?.client ?? existing?.obj ?? {}; const limitIp = input.limitIp ?? existingClient.limitIp ?? 0; const groupName = input.groupName ?? existingClient.group ?? undefined; const body: Record<string, unknown> = { email, totalGB: Number(input.totalBytes), expiryTime: input.expiresAt.getTime(), tgId: Number(input.telegramId), limitIp: Math.max(0, Number(limitIp ?? 0)), enable: true }; if (groupName) body.group = groupName; const res = await request<{ obj?: unknown }>(`/panel/api/clients/update/${encodeURIComponent(email)}`, { method: "POST", body: JSON.stringify(body) }); logger.info("XRAY_CLIENT_RENEW_SUCCESS", { email }); return res.obj; } catch (error) { logger.error("XRAY_CLIENT_RENEW_FAILED", { email, error: sanitizePanelError(error) }); throw error; } }
