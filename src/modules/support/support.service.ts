...
export class SupportService {
  ...
  static async closeTicket(ticketId: string, actorId: string, actorRole: "admin" | "user" = "admin") {
    return prisma.$transaction(async (tx) => {
      const ticket = await tx.ticket.update({ where: { id: ticketId }, data: { status: "closed", updatedAt: new Date() } });
      ...
      await tx.auditLog.create({ data: { actorId, action: "ticket.close", metadata: JSON.stringify({ ticketId, actorRole }) } });
      eventBus.emit("ticket.closed", { ticketId, userId: ticket.userId, actorId, actorRole });
      if (actorRole === "admin") await notificationService.notifyUser(ticket.userId, `✅ تیکت #${shortId(ticketId)} توسط پشتیبانی بسته شد.`);
      else await notificationService.notifyAdmins({ text: `✅ تیکت #${shortId(ticketId)} توسط کاربر بسته شد.`, actions: [[{ text: "👁 مشاهده سابقه", callbackData: ticketAction(ticketId) }]] });
      return ticket;
    });
  }

  static async reopenTicket(ticketId: string, userId: string) {
    const ticket = await prisma.ticket.findFirst({ where: { id: ticketId, userId }, include: { user: true } });
    if (!ticket) throw new Error("تیکت پیدا نشد");
    ...
    const reopened = await prisma.$transaction(async (tx) => {
      ...
      await tx.ticketMessage.create({ data: { ticketId, senderId: userId, senderRole: "user", message: "تیکت دوباره باز شد." } });
      return updated;
    });
    await notificationService.notifyAdmins({ text: `🔄 تیکت #${shortId(ticketId)} دوباره باز شد.\n\n👤 کاربر: ${reopened.user.telegramId}`, actions: [[{ text: "💬 ورود به چت", callbackData: `support:admin:chat:${ticketId}` }]] });
    return reopened;
  }
