...
export const confirmationDialog = (
  ...
  reply_markup: keyboard()
    .row(
      keyboard().button('تأیید', 'confirm', yesAction),
      keyboard().button('انصراف', 'nav', noAction),
    )
    .build(),
});

export const clipboardButton = (label: string, value: string): InlineKeyboardMarkup =>
  keyboard().row(keyboard().url(label, value)).build();

export const toggleButton = (label: string, enabled: boolean, key: string): InlineKeyboardMarkup =>
  keyboard()
    .row(keyboard().button(`${enabled ? '✅' : '◻️'} ${label}`, 'toggle', key, enabled ? '0' : '1'))
    .build();

export const paginatedListKeyboard = (
  page: number,
  hasNext: boolean,
  target: string,
...
export type FinancialStatus = 'pending' | 'success' | 'failed';

export const financialPill = (status: FinancialStatus): string => {
