...
function registerModernHandlers(bot) {
  ...
  async function handleQuickReplyNavigation(ctx, text) {
    ...
    if (target === "refresh") {
      ...
      await (0, panel_ui_1.renderPanel)(ctx, current, "replace");
      return true;
    }
    if (target === "claimFree") {
...
پیام خود را ارسال کنید.`,
        {
          reply_markup: {
            inline_keyboard: [
              [{ text: "✅ بستن تیکت", callback_data: `support:close:${ticket.id}` }],
  ...
  bot.start(async (ctx) => {
    ...
    if (payload) await referral_service_1.ReferralService.linkReferral(user.id, payload);
    await (0, panel_ui_1.renderPanel)(ctx, { id: "home" }, "replace");
  });
  bot.action(/^nav:(.+)$/, async (ctx) => {
    await ctx.answerCbQuery();
    if (ctx.match[1] === "back") return (0, panel_ui_1.goBack)(ctx);
...
این اطلاعات در بخش «اکانت‌های من» نیز همیشه در دسترس است.`,
        {
          reply_markup: {
            inline_keyboard: [
              [
...
${error instanceof Error ? error.message : "در انجام درخواست مشکلی پیش آمد. لطفاً چند لحظه دیگر دوباره تلاش کنید."}`,
        {
          reply_markup: {
            inline_keyboard: [
              [
                { text: "💳 شارژ کیف پول", callback_data: (0, panel_ui_1.callbackFor)("deposit") },
                { text: "⬅️ بازگشت به پیش‌فاکتور", callback_data: "nav:back" },
              ],
              [{ text: "🎧 پشتیبانی", callback_data: (0, panel_ui_1.callbackFor)("support") }],
            ],
          },
...
پس از پرداخت، محصول به صورت خودکار تحویل خواهد شد.`,
        (0, design_system_1.InvoiceActionKeyboard)(invoice.paymentLink ?? "", (0, panel_ui_1.callbackFor)("shop.checkout", { productId })),
      );
      await ctx.reply("می‌توانید با دکمه‌های زیر سریع‌تر بین بخش‌های پرداخت جابه‌جا شوید.", (0, design_system_1.PaymentKeyboard)());
    } catch (error) {
      await ctx.editMessageText(`❌ ${error instanceof Error ? error.message : "ایجاد پرداخت ناموفق بود"}`, {
        reply_markup: { inline_keyboard: [[{ text: "🔙 بازگشت", callback_data: (0, panel_ui_1.callbackFor)("shop.checkout", { productId }) }]] },
      });
    }
  });
  bot.action(/^admin:payment_gateway:status:(enabled|disabled)$/, async (ctx) => {
    await ctx.answerCbQuery();
    if (!ctx.from || !(await (0, admin_middleware_1.isAdminByTelegramId)(ctx.from.id))) return void (await ctx.answerCbQuery("دسترسی غیرمجاز"));
    try {
...
📤 پس از پرداخت، تصویر رسید را همین‌جا ارسال کنید.`,
        { reply_markup: { inline_keyboard: [[{ text: "❌ لغو عملیات", callback_data: "flow:cancel" }]] } },
      );
    } catch (error) {
...
📦 این اکانت به بخش «اکانت‌های من» اضافه شد و در هر زمان می‌توانید اطلاعات آن را مشاهده کنید.`,
      ...
      const keyboard =
        error instanceof free_account_service_1.FreeAccountError && error.code === "ACTIVE_ACCOUNT"
          ? [
              [{ text: "📦 اکانت‌های من", callback_data: (0, panel_ui_1.callbackFor)("account.details") }],
              [{ text: "🏠 منوی اصلی", callback_data: (0, panel_ui_1.callbackFor)("home") }],
            ]
  ...
  bot.action(/^admin:free_account:view:([^:]+)$/, async (ctx) => {
    if (!ctx.from || !(await (0, admin_middleware_1.isAdminByTelegramId)(ctx.from.id))) return ctx.answerCbQuery("دسترسی غیرمجاز");
    await ctx.answerCbQuery();
    ...
    const expiresAt = assignment
      ? (assignment.expiresAt ??
        (0, free_account_service_1.freeAccountExpiresAt)(assignment.assignedAt ?? assignment.createdAt, account.durationDays))
      : undefined;
    await ctx.reply(
      `🆓 جزئیات اکانت تست
...
━━━━━━━━━━━━━━━━`,
      {
        reply_markup: {
          inline_keyboard: [
            ...
            [
              { text: "✅ آماده", callback_data: `admin:free_account:status:${account.id}:available` },
              { text: "🚫 منقضی/غیرفعال", callback_data: `admin:free_account:status:${account.id}:expired` },
            ],
...
پیام خود را ارسال کنید. محدودیتی در تعداد پیام‌ها وجود ندارد.`,
  ...
  bot.action(/^support:chat:([^:]+)$/, async (ctx) => {
    ...
    if (ticket.status === "closed") await support_service_1.SupportService.reopenTicket(ticket.id, user.id, "user");
    ctx.session.liveTicketId = ticket.id;
    ctx.session.liveTicketRole = "user";
    await ctx.reply(
      `💬 گفتگو باز شد
...
پاسخ خود را ارسال کنید. هر پیام جداگانه برای کاربر ارسال می‌شود.`,
        ...
        reply_markup: {
          inline_keyboard: [
            [
              ...
              { text: "✅ بستن", callback_data: `admin:ticket:close:${ticket.id}` },
            ],
  ...
  bot.action(/^admin:category:hard_delete:confirm:([^:]+)$/, async (ctx) => {
    ...
    await ctx.reply("⚠️ حذف دائمی دسته‌بندی غیرقابل بازگشت است و محصولات وابسته را هم حذف می‌کند.", {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "تایید حذف دائمی", callback_data: `admin:category:hard_delete:force:${ctx.match[1]}` },
  ...
  bot.action(/^admin:account:delete:confirm:([^:]+)$/, async (ctx) => {
    ...
    await ctx.reply("⚠️ این اکانت از موجودی حذف شود؟", {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "تایید حذف", callback_data: `admin:account:delete:force:${ctx.match[1]}` },
  ...
  bot.action(/^admin:wallet:delete:confirm:([^:]+)$/, async (ctx) => {
    ...
    await ctx.reply("⚠️ این کیف پول حذف شود؟ اگر پرداخت فعال داشته باشد حذف انجام نمی‌شود.", {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "تایید حذف", callback_data: `admin:wallet:delete:force:${ctx.match[1]}` },
  ...
  bot.action(/^admin:coupon:(soft_delete|hard_delete):([^:]+)$/, async (ctx) => {
    if (!ctx.from || !(await (0, admin_middleware_1.isAdminByTelegramId)(ctx.from.id))) return ctx.answerCbQuery("دسترسی غیرمجاز");
    await ctx.answerCbQuery();
    if (ctx.match[1] === "soft_delete") await coupon_service_1.CouponService.softDelete(ctx.match[2], String(ctx.from.id));
    else await coupon_service_1.CouponService.hardDelete(ctx.match[2], String(ctx.from.id));
  ...
  bot.action(/^admin:product:hard_delete:confirm:([^:]+)$/, async (ctx) => {
    ...
    await ctx.reply("⚠️ حذف دائمی محصول غیرقابل بازگشت است. اگر محصول سفارش فعال داشته باشد با تایید نهایی هم حذف می‌شود.", {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "تایید حذف دائمی", callback_data: `admin:product:hard_delete:force:${ctx.match[1]}` },
  ...
  bot.action(/^admin:deposit:(approve|reject):(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery();
    try {
      if (ctx.match[1] === "approve") await deposit_service_1.DepositService.approve(ctx.match[2], String(ctx.from.id));
      else await deposit_service_1.DepositService.reject(ctx.match[2], String(ctx.from.id));
    } catch (error) {
  ...
  bot.on("photo", async (ctx, next) => {
    ...
    if (photo && (await (0, flow_engine_1.handleActiveFlowPhoto)(ctx, photo.file_id))) return;
    return next();
  });
  bot.on("text", async (ctx, next) => {
    const text = ctx.message.text.trim();
    if (await handleQuickReplyNavigation(ctx, text)) return;
    if (await (0, flow_engine_1.handleActiveFlowText)(ctx, text)) return;
    if (ctx.session.liveTicketId && ctx.session.liveTicketRole) {
      try {
        if (ctx.session.liveTicketRole === "admin") {
          if (!ctx.from || !(await (0, admin_middleware_1.isAdminByTelegramId)(ctx.from.id))) return next();
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  ...
                  { text: "✅ بستن", callback_data: `admin:ticket:close:${ctx.session.liveTicketId}` },
                ],
              ],
            },
          });
        ...
        await ctx.reply("📩 پیام شما ارسال شد. برای ادامه گفتگو، پیام بعدی را ارسال کنید.", {
          reply_markup: {
            inline_keyboard: [
              [{ text: "✅ بستن تیکت", callback_data: `support:close:${ctx.session.liveTicketId}` }],
              [{ text: "🏠 منوی اصلی", callback_data: (0, panel_ui_1.callbackFor)("home") }],
            ],
          },
        });
