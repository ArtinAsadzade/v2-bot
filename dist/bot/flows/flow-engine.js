...
لینک عضویت معتبر را وارد کنید:` : "⚠️ ذخیره کانال ناموفق بود. لینک عضویت را دوباره وارد کنید:" };
    ...
    payment_gateway_update: {
        ...
        async handleText(ctx, text) {
            ...
            const payload = {};
            if (field === "apiBaseUrl")
                payload.apiBaseUrl = text;
            else if (field === "apiKey")
                payload.apiKey = text;
            else if (field === "callbackUrl")
                payload.callbackUrl = text;
            else if (field === "gatewayName")
                payload.gatewayName = text;
            else {
                const data = parseKeyValueLines(text);
            ...
            try {
                await payment_service_1.PaymentGatewayService.updateConfig(payload, String(ctx.from?.id ?? "admin"));
