...
const freeAccountExpiry = (item) =>
  item.expiresAt ?? (0, free_account_service_1.freeAccountExpiresAt)(item.assignedAt ?? item.createdAt, item.account.durationDays);
...
function registerModernViews() {
  (0, panel_ui_1.registerView)("home", async (ctx) => {
    ...
    if (isAdmin) keyboard.push([{ text: "⚙️ مرکز مدیریت", action: (0, panel_ui_1.callbackFor)("admin.dashboard") }]);
    return {
...
لطفاً روش پرداخت را انتخاب کنید.`,
  ...
  (0, panel_ui_1.registerView)("account", async (ctx) => {
    ...
    return {
      ...
      keyboard: [
        [
          { text: "🛒 خرید سرویس", action: (0, panel_ui_1.callbackFor)("shop.categories") },
          { text: "💳 شارژ کیف پول", action: (0, panel_ui_1.callbackFor)("deposit") },
        ],
        [
          { text: "📦 اکانت‌های من", action: (0, panel_ui_1.callbackFor)("account.details") },
...
ماژول مدیریتی را انتخاب کنید:`,
      keyboard: [
        [
          { text: "📊 آمار", action: (0, panel_ui_1.callbackFor)("admin.analytics") },
        ...
        [
          ...
          { text: "🎁 رفرال", action: (0, panel_ui_1.callbackFor)("admin.referrals") },
        ],
        [
  ...
  (0, panel_ui_1.registerView)("admin.categories", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: [
        ...
        ...categories.map((category) => [
          {
            text: `${category.icon ?? "📂"} مدیریت ${category.name}`,
            action: (0, panel_ui_1.callbackFor)("admin.category", { categoryId: category.id }),
          },
        ]),
        [
...
${detail.products.map((product) => `• ${product.title} · ${product.isActive ? "فعال" : "غیرفعال"} · فروش ${product._count.orders.toLocaleString("fa-IR")}`).join("\n") || "محصولی در این دسته نیست."}`,
  ...
  (0, panel_ui_1.registerView)("admin.accounts", async (_ctx, params) => {
    const current = page(params);
    const status = ["available", "reserved", "sold", "disabled", "expired"].includes(params.status) ? params.status : undefined;
  ...
  تاریخ تخصیص: ${account.assignedDate ? account.assignedDate.toLocaleString("fa-IR") : "—"}`,
      ...
      keyboard: [
        ...
        ...accounts.map((account) => [
          { text: `👁 ${account.username}`, action: (0, panel_ui_1.callbackFor)("admin.account", { accountId: account.id }) },
        ]),
...
${inventory.map((item) => `• ${item.username} · ${item.durationDays.toLocaleString("fa-IR")} روز · ${free_account_service_1.FREE_ACCOUNT_STATUS_LABELS[item.status]}`).join("\n") || "موجودی ثبت نشده است."}`,
      keyboard: [
        ...
        ...inventory.map((item) => [
          {
            text: `👁 ${item.username} · ${free_account_service_1.FREE_ACCOUNT_STATUS_LABELS[item.status]}`,
            action: `admin:free_account:view:${item.id}`,
          },
        ]),
        [
...
بخش موردنظر را انتخاب کنید:`,
  ...
  (0, panel_ui_1.registerView)("admin.notifications", async () => {
    ...
    const recentLines =
      recent
        .map(
          (item) => `• ${item.createdAt.toLocaleString("fa-IR")} · ${item.targetLabel}
...
بخش تنظیمات را انتخاب کنید:`,
  ...
  (0, panel_ui_1.registerView)("admin.paymentGateway", async () => {
    const [gateway, stats] = await Promise.all([
      payment_service_1.PaymentGatewayService.getConfig(),
      payment_service_1.PaymentInvoiceService.stats(),
    ]);
    ...
    const lastTest =
      gateway.lastSuccessfulRequest && gateway.lastFailedRequest
        ? gateway.lastSuccessfulRequest > gateway.lastFailedRequest
          ? gateway.lastSuccessfulRequest
          : gateway.lastFailedRequest
        : (gateway.lastSuccessfulRequest ?? gateway.lastFailedRequest);
    return {
      replyKeyboard: "admin",
...
${stats.recent.map((invoice) => `• #${shortId(invoice.id)} · ${invoice.user.telegramId} · ${invoice.status} · ${money(invoice.amount)}`).join("\n") || "فاکتوری ثبت نشده است."}`,
  ...
  (0, panel_ui_1.registerView)("admin.invoices", async (_ctx, params) => {
    ...
    const statusLabel = (value) =>
      ({ PENDING: "در انتظار", PAID: "پرداخت شده", CANCELED: "لغو شده", FAILED: "ناموفق", COMPLETED: "پرداخت شده" })[value] ?? value;
    ...
    .join("\n\n") || "فاکتوری ثبت نشده است."
      ...
      keyboard: [
        ...
        ...invoices.map((invoice) => [
          { text: `👁 #${shortId(invoice.id)}`, action: (0, panel_ui_1.callbackFor)("admin.invoice", { invoiceId: invoice.id }) },
        ]),
        [
