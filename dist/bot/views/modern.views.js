...
function registerModernViews() {
  (0, panel_ui_1.registerView)("home", async (ctx) => {
    ...
    const keyboard = [
      ...
      [
        { text: "➕ شارژ حساب", action: (0, panel_ui_1.callbackFor)("deposit") },
    ...
    if (isAdmin) keyboard.push([{ text: "⚙️ پنل مدیریت", action: (0, panel_ui_1.callbackFor)("admin.dashboard") }]);
    return {
  ...
  (0, panel_ui_1.registerView)("shop.product", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: [
        [{ text: "🛒 خرید", action: (0, panel_ui_1.callbackFor)("shop.checkout", { productId: product.id }) }],
        [{ text: "🎟 ثبت کد تخفیف", action: `flow:start:coupon_code:${product.id}` }],
      ],
    };
  });
  (0, panel_ui_1.registerView)("shop.checkout", async (ctx, params) => {
    ...
    return {
      text: `🧾 پیش‌فاکتور\n\nمحصول: ${product.title}\nقیمت: ${money(product.price)}\nکد تخفیف: ${coupon ?? "ثبت نشده"}\nموجودی کیف پول: ${money(user.balance)}\n\nبرای تکمیل خرید تایید کنید.`,
      keyboard: [
        [{ text: "✅ تایید خرید", action: `buy:confirm:${product.id}` }],
        [{ text: "🎟 ثبت کد تخفیف", action: `flow:start:coupon_code:${product.id}` }],
      ],
    };
  });
  (0, panel_ui_1.registerView)("wallet", async (ctx) => {
    const user = ctx.from ? await user_service_1.UserService.getByTelegramId(ctx.from.id) : undefined;
    return {
      text: `💳 کیف پول\n\nموجودی فعلی: ${money(user?.balance ?? 0)}\n\nبرای افزایش موجودی، شارژ حساب را انتخاب کنید.`,
      keyboard: [[{ text: "➕ شارژ حساب", action: (0, panel_ui_1.callbackFor)("deposit") }]],
    };
  });
  (0, panel_ui_1.registerView)("deposit", async () => ({
    text: "➕ شارژ حساب\n\nمبلغ شارژ را در یک مرحله امن وارد کنید. بعد از ایجاد درخواست، رسید را ارسال می‌کنید.",
  ...
  (0, panel_ui_1.registerView)("support", async (ctx) => {
    ...
    void tickets;
    return {
      text: "🎧 پشتیبانی\n\nبرای ارتباط با تیم پشتیبانی، یک تیکت جدید بسازید یا پاسخ خود را در جریان تیکت ارسال کنید.",
      keyboard: [[{ text: "✉️ ایجاد تیکت", action: "flow:start:ticket_reply" }]],
    };
  });
  ...
  (0, panel_ui_1.registerView)("admin.dashboard", async () => {
    ...
    return {
      ...
      keyboard: [
        ...
        [
          { text: "🔐 اکانت‌ها", action: (0, panel_ui_1.callbackFor)("admin.accounts") },
          { text: "🎁 اکانت رایگان", action: (0, panel_ui_1.callbackFor)("admin.freeAccounts") },
        ],
        [
          { text: "🎟 کوپن‌ها", action: (0, panel_ui_1.callbackFor)("admin.coupons") },
  ...
  (0, panel_ui_1.registerView)("admin.users", async (_ctx, params) => {
    ...
    keyboard.push([
      { text: "قبلی", action: (0, panel_ui_1.callbackFor)("admin.users", { page: Math.max(current - 1, 1) }) },
      { text: "بعدی", action: (0, panel_ui_1.callbackFor)("admin.users", { page: current + 1 }) },
    ]);
  ...
  (0, panel_ui_1.registerView)("admin.products", async (_ctx, params) => {
    ...
    keyboard.push([
      { text: "قبلی", action: (0, panel_ui_1.callbackFor)("admin.products", { page: Math.max(current - 1, 1) }) },
      { text: "بعدی", action: (0, panel_ui_1.callbackFor)("admin.products", { page: current + 1 }) },
    ]);
  ...
  (0, panel_ui_1.registerView)("admin.accounts", async () => {
    ...
    return {
      text: `🔐 مدیریت اکانت‌ها\n\nاکانت‌های آماده فروش: ${stats.available.toLocaleString("fa-IR")}\nاکانت‌های فروخته‌شده: ${stats.sold.toLocaleString("fa-IR")}\n\nبرای افزودن اکانت، محصول را انتخاب کنید.`,
      keyboard: stats.products.map((product) => [{ text: `➕ ${product.title}`, action: `flow:start:account_create:${product.id}` }]),
    };
  });
  (0, panel_ui_1.registerView)("admin.freeAccounts", async () => {
    const stats = await free_account_service_1.FreeAccountService.stats();
    return {
      text: `🎁 استخر اکانت رایگان\n\nآماده تخصیص: ${stats.available.toLocaleString("fa-IR")}\nتخصیص‌یافته: ${stats.assigned.toLocaleString("fa-IR")}\nآستانه دعوت: ${free_account_service_1.FreeAccountService.threshold().toLocaleString("fa-IR")} نفر\n\nبرای افزودن اکانت رایگان، محصول را انتخاب کنید.`,
      keyboard: stats.products.map((product) => [{ text: `➕ ${product.title}`, action: `flow:start:free_account_create:${product.id}` }]),
    };
  });
  (0, panel_ui_1.registerView)("admin.coupons", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: [
        ...
        [
          { text: "قبلی", action: (0, panel_ui_1.callbackFor)("admin.coupons", { page: Math.max(current - 1, 1) }) },
          { text: "بعدی", action: (0, panel_ui_1.callbackFor)("admin.coupons", { page: current + 1 }) },
        ],
      ],
    };
  });
  (0, panel_ui_1.registerView)("admin.deposits", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: deposits.map((deposit) => [
        {
          text: `💳 ${deposit.user.telegramId} — ${money(deposit.amount)}`,
          action: (0, panel_ui_1.callbackFor)("admin.deposit", { depositId: deposit.id }),
        },
      ]),
    };
  });
  (0, panel_ui_1.registerView)("admin.deposit", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: [
        [
          { text: "✅ تایید", action: `admin:deposit:approve:${deposit.id}` },
  ...
  (0, panel_ui_1.registerView)("admin.orders", async (_ctx, params) => {
    ...
    return {
      ...
      keyboard: [
        [
          { text: "قبلی", action: (0, panel_ui_1.callbackFor)("admin.orders", { page: Math.max(current - 1, 1) }) },
          { text: "بعدی", action: (0, panel_ui_1.callbackFor)("admin.orders", { page: current + 1 }) },
        ],
      ],
    };
  });
  (0, panel_ui_1.registerView)("admin.tickets", async (_ctx, params) => {
    ...
    return {
      text: `🎧 تیکت‌های فعال\n\nصفحه ${current.toLocaleString("fa-IR")} از ${pages(total, 8)}`,
      keyboard: tickets.map((ticket) => [
        {
          text: `🎧 ${ticket.user.telegramId} — ${ticket.id.slice(-6)}`,
