...
توکن کامل هرگز نمایش داده نمی‌شود.`,
    ...
    (0, panel_ui_1.registerView)("admin.xrayClients", async (_ctx, params) => {
        ...
        const product = productId
            ? await admin_service_1.AdminService.productDetail(productId)
...
${clients
                .map((client) => `• ${client.telegramId} · ${client.isFreeTest ? "🆓 اکانت تست" : (client.product?.title ?? "سرویس Xray")}
...
${client.status === "missing_on_panel" ? "حذف‌شده از پنل / نیازمند بررسی\n" : ""}`)
            ...
            keyboard: [
                [
                    { text: "همه", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams()) },
                    { text: "فعال", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams("active")) },
                ],
                [
                    { text: "در حال ساخت", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams("provisioning")) },
                    { text: "ناموفق", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams("failed")) },
                ],
                [
                    { text: "منقضی", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams("expired")) },
                    { text: "حذف‌شده از پنل / نیازمند بررسی", action: (0, panel_ui_1.callbackFor)("admin.xrayClients", filterParams("missing_on_panel")) },
                ],
...
اگر سوالی دارید، پشتیبانی در کنار شماست.`,
    ...
    (0, panel_ui_1.registerView)("shop.product", async (ctx, params) => {
        ...
        if (!product)
            return {
                text: (0, messages_1.errorMessage)("محصول در دسترس نیست", "این محصول در حال حاضر قابل خرید نیست.", "لطفاً محصول دیگری را انتخاب کنید."),
                keyboard: [],
            };
    ...
    (0, panel_ui_1.registerView)("shop.checkout", async (ctx, params) => {
        ...
        if (couponLine)
            keyboard.push([
                { text: "🗑 حذف کد تخفیف", action: (0, panel_ui_1.actionFor)("coupon:remove", product.id) },
                { text: "🎟 تغییر کد تخفیف", action: (0, panel_ui_1.actionFor)("coupon:change", product.id) },
            ]);
        else
...
برای مدیریت حساب، یکی از بخش‌های زیر را انتخاب کنید.`,
    ...
    (0, panel_ui_1.registerView)("account.details", async (ctx) => {
        ...
        const freeXrayClients = await prisma_1.prisma.xrayClient.findMany({
        ...
        return {
            ...
            keyboard: [
                ...
                client.isFreeTest
                    ...
                    : [
                        { text: "⚙️ دریافت کانفیگ‌ها", action: `xray:configs:${client.id}` },
                        { text: "🔄 تمدید سرویس", action: (0, panel_ui_1.callbackFor)("account.renew", { xrayClientId: client.id }) },
                    ],
                [
                    { text: "📊 بروزرسانی اطلاعات", action: (0, panel_ui_1.callbackFor)("account.xray", { xrayClientId: client.id }) },
                    { text: "🎫 پشتیبانی", action: (0, panel_ui_1.callbackFor)("support") },
                ],
            ],
        };
    ...
    (0, panel_ui_1.registerView)("account.renew", async (ctx, params) => {
        ...
        const client = await prisma_1.prisma.xrayClient.findFirst({
        ...
        if (!client)
            return {
                ...
                keyboard: [[{ text: "🔙 بازگشت", action: (0, panel_ui_1.callbackFor)("account.details") }]],
                navigation: { back: false, home: false },
            };
        ...
        const rows = categories.length === 1
            ? categories[0].products.map((product) => [
                {
                    text: product.title,
                    action: (0, callback_tokens_1.tokenAction)("xr:r:s", (0, callback_tokens_1.createCallbackToken)(ctx, "renewal", { xrayClientId: client.id, productId: product.id })),
                },
            ])
            : categories.map((category) => [
                {
                    text: `📂 ${category.name}`.slice(0, 60),
                    action: (0, panel_ui_1.callbackFor)("account.renew.products", { xrayClientId: client.id, categoryId: category.id }),
                },
            ]);
        if (rows.length === 0) {
            return {
...
در حال حاضر پلنی برای تمدید موجود نیست.`,
                keyboard: [
                    [{ text: "🛒 فروشگاه", action: (0, panel_ui_1.callbackFor)("shop.categories") }],
                    [{ text: "🎫 پشتیبانی", action: (0, panel_ui_1.callbackFor)("support") }],
                    [{ text: "🔙 بازگشت", action: (0, panel_ui_1.callbackFor)("account.xray", { xrayClientId: client.id }) }],
                ],
                navigation: { back: false, home: false },
            };
        }
...
لطفاً پلن تمدید را انتخاب کنید:`,
            keyboard: [...rows, [{ text: "🔙 بازگشت", action: (0, panel_ui_1.callbackFor)("account.xray", { xrayClientId: client.id }) }]],
            navigation: { back: false, home: false },
        };
    });
...
در حال حاضر پلنی برای تمدید موجود نیست.`,
                keyboard: [
                    [{ text: "🛒 فروشگاه", action: (0, panel_ui_1.callbackFor)("shop.categories") }],
                    [{ text: "🎫 پشتیبانی", action: (0, panel_ui_1.callbackFor)("support") }],
                    [{ text: "🔙 بازگشت", action: (0, panel_ui_1.callbackFor)("account.renew", { xrayClientId: client.id }) }],
                ],
                navigation: { back: false, home: false },
            };
        }
...
لطفاً پلن تمدید را انتخاب کنید:`,
            keyboard: [
                ...available.map((p) => [
                    { text: p.title, action: (0, callback_tokens_1.tokenAction)("xr:r:s", (0, callback_tokens_1.createCallbackToken)(ctx, "renewal", { xrayClientId: client.id, productId: p.id })) },
                ]),
                [{ text: "🔙 بازگشت", action: (0, panel_ui_1.callbackFor)("account.renew", { xrayClientId: client.id }) }],
            ],
            navigation: { back: false, home: false },
...
${money(quote.product.price)}${quote.liveOk ? "" : "\n\n⚠️ اطلاعات لحظه‌ای پنل در دسترس نبود؛ محاسبه با داده محلی انجام شد."}`,
                ...
                [
                    {
                        ...
                        action: (0, callback_tokens_1.tokenAction)("xr:r:w", (0, callback_tokens_1.createCallbackToken)(ctx, "renewal", { xrayClientId: quote.client.id, productId: quote.product.id })),
                    },
                    {
                        text: "⚡ پرداخت آنی",
                        action: (0, callback_tokens_1.tokenAction)("xr:r:i", (0, callback_tokens_1.createCallbackToken)(ctx, "renewal", { xrayClientId: quote.client.id, productId: quote.product.id })),
                    },
                ],
                [
                    {
                        text: "🔙 بازگشت",
                        action: (0, panel_ui_1.callbackFor)("account.renew.products", { xrayClientId: quote.client.id, categoryId: quote.product.categoryId }),
                    },
                ],
            ],
            navigation: { back: false, home: false },
    ...
    (0, panel_ui_1.registerView)("wallet", async (ctx) => {
        const user = ctx.from ? await user_service_1.UserService.getByTelegramId(ctx.from.id) : undefined;
        const dashboard = user ? await user_service_1.UserService.dashboard(user.id) : undefined;
        const recent = dashboard?.walletTransactions
            .slice(0, 3)
            .map((tx) => `• ${tx.type === "credit" || tx.type === "transfer_in" ? "افزایش" : "کاهش"}: ${money(tx.amount)} · ${tx.createdAt.toLocaleDateString("fa-IR")}`)
...
برای مدیریت، وارد یکی از گروه‌های اصلی شوید.`,
            keyboard: [
                [
                    { text: "🛒 فروشگاه", action: (0, panel_ui_1.callbackFor)("admin.store") },
                    { text: "💳 مالی", action: (0, panel_ui_1.callbackFor)("admin.finance") },
                ],
                [
                    { text: "👥 کاربران و پشتیبانی", action: (0, panel_ui_1.callbackFor)("admin.usersSupport") },
                    { text: "🛡 مانیتورینگ", action: (0, panel_ui_1.callbackFor)("admin.monitoring") },
                ],
...
مدیریت محصولات، دسته‌بندی‌ها، موجودی اکانت‌ها، اکانت تست و راهنمای محصولات از این بخش انجام می‌شود.`,
            keyboard: [
                [
                    ...
                    { text: "📂 دسته‌بندی‌ها", action: (0, panel_ui_1.callbackFor)("admin.categories") },
                ],
                [
                ...
                [
                    ...
                    { text: "🆓 اکانت تست", action: (0, panel_ui_1.callbackFor)("admin.freeAccounts") },
                ],
...
مدیریت همه ابزارهای مالی از این زیرمنو انجام می‌شود.`,
            keyboard: [
                [
                    { text: "⚡ پرداخت آنی", action: (0, panel_ui_1.callbackFor)("admin.paymentGateway") },
                    { text: "💎 واریزی‌های رمزارزی", action: (0, panel_ui_1.callbackFor)("admin.deposits") },
                ],
                [
                    { text: "💳 کیف پول‌ها", action: (0, panel_ui_1.callbackFor)("admin.wallets") },
                    { text: "🎟 کدهای تخفیف", action: (0, panel_ui_1.callbackFor)("admin.coupons") },
                ],
                [
...
بخش موردنظر را انتخاب کنید.`,
            keyboard: [
                [
                    { text: "👥 مدیریت کاربران", action: (0, panel_ui_1.callbackFor)("admin.users") },
                    { text: "🎫 تیکت‌ها", action: (0, panel_ui_1.callbackFor)("admin.tickets") },
                ],
                [
                    { text: "🎁 پاداش دعوت", action: (0, panel_ui_1.callbackFor)("admin.referrals") },
                    { text: "📊 گزارش کاربران", action: (0, panel_ui_1.callbackFor)("admin.analytics") },
                ],
                [{ text: "📢 اطلاع رسانی و محتوا", action: (0, panel_ui_1.callbackFor)("admin.content") }],
            ],
        };
    });
...
ارسال اطلاعیه، راهنمای محصولات و نمایش عمومی پلن‌ها در این بخش گروه‌بندی شده‌اند.`,
            keyboard: [
                [
                    { text: "📢 اطلاع‌رسانی", action: (0, panel_ui_1.callbackFor)("admin.notifications") },
                    { text: "📘 راهنمای محصولات", action: (0, panel_ui_1.callbackFor)("admin.productGuides") },
                ],
...
یادداشت: تغییر یوزرنیم فقط از طریق BotFather امکان‌پذیر است.`,
            keyboard: [
                [
                    { text: "🏷 نام ربات", action: (0, panel_ui_1.callbackFor)("admin.botSettings") },
                    { text: "📝 توضیحات", action: (0, panel_ui_1.callbackFor)("admin.botSettings") },
                ],
                [
                    { text: "🖼 عکس پروفایل", action: (0, panel_ui_1.callbackFor)("admin.botSettings") },
                    { text: "👤 یوزرنیم", action: (0, panel_ui_1.callbackFor)("admin.botSettings") },
                ],
                [
                    { text: "🏪 وضعیت فروشگاه", action: (0, panel_ui_1.callbackFor)("admin.settings") },
    ...
    (0, panel_ui_1.registerView)("admin.monitoring", async () => {
        ...
        const recentErrors = monitoring.events
            .slice(0, 5)
...
آخرین پرداخت ناموفق: ${monitoring.lastFailedPayment?.updatedAt ? monitoring.lastFailedPayment.updatedAt.toLocaleString("fa-IR") : "—"}`,
            keyboard: [
                [
                    { text: "🚨 خطاهای اخیر", action: (0, panel_ui_1.callbackFor)("admin.monitoring") },
                    { text: "💳 خطاهای پرداخت", action: (0, panel_ui_1.callbackFor)("admin.paymentStats") },
                ],
                [
                    { text: "🎫 خطاهای تیکت", action: (0, panel_ui_1.callbackFor)("admin.tickets") },
                    { text: "⚙️ وضعیت سرویس‌ها", action: (0, panel_ui_1.callbackFor)("admin.monitoring") },
                ],
  ...
  موجودی: ${product.inventoryCount.toLocaleString("fa-IR")} · فروخته‌شده: ${product.soldCount.toLocaleString("fa-IR")} · فعال: ${product.activeCount.toLocaleString("fa-IR")}`)
    ...
    (0, panel_ui_1.registerView)("admin.product", async (ctx, params) => {
        ...
        const inboundSnapshot = detail.product.inboundSnapshot
            ? JSON.parse(detail.product.inboundSnapshot)
            : [];
        if (isXray) {
            return {
...
کلاینت‌های قبلی تغییر نمی‌کنند.`,
                keyboard: [
                    ...
                    [
                        {
                            text: "👥 تغییر گروه",
                            action: (0, callback_tokens_1.tokenAction)("xpg:l:pe", (0, callback_tokens_1.createCallbackToken)(ctx, "xrayPickerProduct", { target: "product_edit", productId: detail.product.id })),
                        },
                        {
                            text: "🔗 تغییر اینباندها",
                            action: (0, callback_tokens_1.tokenAction)("xpi:l:pe", (0, callback_tokens_1.createCallbackToken)(ctx, "xrayPickerProduct", { target: "product_edit", productId: detail.product.id })),
                        },
                    ],
    ...
    (0, panel_ui_1.registerView)("admin.accounts", async (_ctx, params) => {
        ...
        return {
            text: `🗄 مدیریت موجودی اکانت‌ها\n\nکل: ${stats.total.toLocaleString("fa-IR")} · آماده: ${stats.available.toLocaleString("fa-IR")} · رزرو: ${stats.reserved.toLocaleString("fa-IR")} · فروخته: ${stats.sold.toLocaleString("fa-IR")} · غیرفعال: ${stats.disabled.toLocaleString("fa-IR")} · منقضی: ${stats.expired.toLocaleString("fa-IR")}\n${status ? `\nفیلتر وضعیت: ${accountStatusLabel(status)}` : ""}\n\nصفحه ${current.toLocaleString("fa-IR")} از ${pages(total, 8)}\n\n${accounts
                .map((account) => `• ${account.username} · ${account.product.title}
...
${sections
                .map((section, index) => `${index + 1}. ${section.icon} ${section.title}
...
`,
    ...
    (0, panel_ui_1.registerView)("admin.coupon", async (_ctx, params) => {
        ...
        const activeLabel = direct.status === "active" && !expired && !direct.deletedAt
            ? "فعال ✅"
            : expired
                ? "⛔ منقضی شده"
...
بخش تنظیمات را انتخاب کنید:`,
    ...
    (0, panel_ui_1.registerView)("admin.paymentGateway", async () => {
        ...
        const lastActualTestStatus = gateway.lastConnectionStatus === "success"
            ? "آخرین تست موفق"
...
${money(stats.monthlyRevenue)}`,
            keyboard: [
                ...
                [
                    { text: "🧾 فاکتورها", action: (0, panel_ui_1.callbackFor)("admin.invoices") },
                    { text: "📊 آمار پرداخت‌ها", action: (0, panel_ui_1.callbackFor)("admin.paymentStats") },
                ],
                [
                    { text: "💎 شارژ رمزارزی", action: (0, panel_ui_1.callbackFor)("admin.deposits") },
                    { text: "💰 تراکنش‌ها", action: (0, panel_ui_1.callbackFor)("admin.transactions") },
                    { text: "💳 کیف پول‌ها", action: (0, panel_ui_1.callbackFor)("admin.wallets") },
                ],
...
${invoices
                .map((invoice) => `• شناسه: #${shortId(invoice.id)}
