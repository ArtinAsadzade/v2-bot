...
class XrayClientService {
    ...
    static async verifyPanelClient(input) { logger_1.logger.info("XRAY_CLIENT_VERIFY_REQUEST", { email: input.email }); const detail = await this.getClient(input.email); const obj = detail?.obj; const client = obj?.client ?? (Array.isArray(obj) ? obj.find((item) => item?.email === input.email || item?.client?.email === input.email) : obj); if (!client || (client.email && client.email !== input.email && client.client?.email !== input.email))
        throw new Error("کلاینت در پنل تایید نشد"); const panelClientId = client.uuid ?? client.id ?? client.clientId ?? client.client?.uuid ?? client.client?.id; const subId = client.subId ?? client.sub_id ?? client.client?.subId ?? client.client?.sub_id; if (!panelClientId && !subId)
        throw new Error("شناسه کلاینت/اشتراک در پنل معتبر نیست"); if (input.requireLinks && subId)
        await this.subLinks(String(subId)); logger_1.logger.info("XRAY_CLIENT_VERIFY_SUCCESS", { email: input.email, panelClientId, subId }); return { exists: true, detail, panelClientId: panelClientId ? String(panelClientId) : undefined, subId: subId ? String(subId) : undefined }; }
