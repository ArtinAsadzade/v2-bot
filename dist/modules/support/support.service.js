...
class SupportService {
    ...
    static async closeTicket(ticketId, actorId, actorRole = "admin") {
        return prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (actorRole === "admin")
                await notification_service_1.notificationService.notifyUser(ticket.userId, `✅ تیکت #${shortId(ticketId)} توسط پشتیبانی بسته شد.`);
            else
    ...
    static async reopenTicket(ticketId, userId) {
        const ticket = await prisma_1.prisma.ticket.findFirst({ where: { id: ticketId, userId }, include: { user: true } });
        if (!ticket)
            throw new Error("تیکت پیدا نشد");
        ...
        await notification_service_1.notificationService.notifyAdmins({ text: `🔄 تیکت #${shortId(ticketId)} دوباره باز شد.\n\n👤 کاربر: ${reopened.user.telegramId}`, actions: [[{ text: "💬 ورود به چت", callbackData: `support:admin:chat:${ticketId}` }]] });
        return reopened;
    }
