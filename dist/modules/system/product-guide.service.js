...
exports.ProductGuideService = void 0;
...
function clean(input) {
    ...
    if (!title || !shortDescription || !body)
        throw new Error("عنوان، توضیح کوتاه و متن راهنما الزامی است");
    if (title.length > TITLE_MAX)
        throw new Error("عنوان راهنما بیش از حد طولانی است");
    if (shortDescription.length > SHORT_MAX)
        throw new Error("توضیح کوتاه راهنما بیش از حد طولانی است");
    if (body.length > BODY_MAX)
        throw new Error("متن راهنما باید کوتاه و قابل خواندن باشد");
...
class ProductGuideService {
    static listActive() {
        return prisma_1.prisma.productGuideSection.findMany({ where: { isActive: true }, orderBy: [{ displayOrder: "asc" }, { createdAt: "asc" }] });
    }
    static listAll() {
        return prisma_1.prisma.productGuideSection.findMany({ orderBy: [{ displayOrder: "asc" }, { createdAt: "desc" }] });
    }
    static save(input, actorId, id) {
    ...
    static setActive(id, isActive, actorId) {
        return prisma_1.prisma.$transaction(async (tx) => {
...
exports.ProductGuideService = ProductGuideService;
