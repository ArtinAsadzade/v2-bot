...
const availableInventoryWhere = { status: "available", assignedTo: null, reservedBy: null, assignment: { is: null } };
class FreeAccountService {
    ...
    static async xrayEligibility(userId) {
        ...
        const [user, last, active] = await Promise.all([
            prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { isBanned: true } }),
            prisma_1.prisma.xrayClient.findFirst({ where: { userId, isFreeTest: true, status: { in: ["active", "provisioning", "creating"] } }, orderBy: { createdAt: "desc" } }),
    ...
    static async claimXray(userId) {
        ...
        try {
            reserved = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (config.usedCount >= config.stockLimit)
            ...
            const verified = await xray_service_1.XrayClientService.verifyPanelClient({ email: reserved.clientEmail, expectedInboundIds: reserved.inboundIds });
            const delivered = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (reserved?.id) {
                ...
                await prisma_1.prisma.auditLog.create({ data: { actorId: userId, action: "free_xray.failed", metadata: JSON.stringify({ xrayClientId: reserved.id, deliveryId: reserved.id, error: message }) } });
                monitoring_service_1.MonitoringService.record({ type: "PAYMENT_DELIVERY_FAILED", section: "Free Test", description: message, userId, severity: "critical", suggestedAction: "اکانت تست در پنل ساخته/verify نشده؛ سهمیه ماهانه مصرف نشده است.", metadata: { xrayClientId: reserved.id } });
            }
            throw error;
        }
    }
