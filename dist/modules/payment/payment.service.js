...
exports.PaymentInvoiceService = exports.PaymentGatewayService = void 0;
...
class PaymentInvoiceService {
    ...
    static async createProductInvoice(userId, productId) {
        ...
        const stock = await product_service_1.ProductService.availableStock(productId);
        if (stock < 1)
    ...
    static async createInvoice(data) {
        ...
        await prisma_1.prisma.paymentAuditLog.create({ data: { userId: data.userId, invoiceId: invoice.id, action: "Invoice Created", metadata: JSON.stringify({ type: data.type, amount: data.amount }) } });
        ...
        try {
            ...
            return prisma_1.prisma.paymentInvoice.update({
                where: { id: invoice.id },
            ...
            await prisma_1.prisma.paymentAuditLog.create({ data: { userId: data.userId, invoiceId: invoice.id, action: "Payment Failed", metadata: JSON.stringify({ stage: "create", error: error instanceof Error ? error.message : String(error) }) } });
    ...
    static async processCallback(invoiceId, metadata = {}) {
        ...
        if (!invoice)
            return { statusCode: 404, text: "Payment invoice not found." };
        await prisma_1.prisma.paymentAuditLog.create({ data: { userId: invoice.userId, invoiceId: invoice.id, action: "Callback Received", metadata: JSON.stringify(metadata) } });
        if (metadata.token !== invoice.callbackToken) {
            await prisma_1.prisma.paymentAuditLog.create({ data: { userId: invoice.userId, invoiceId: invoice.id, action: "Payment Failed", metadata: JSON.stringify({ stage: "callback_security", reason: "invalid_callback_token" }) } });
            return { statusCode: 403, text: "Invalid payment callback." };
        }
        if (invoice.status === "COMPLETED")
            return { statusCode: 200, text: "Payment already processed." };
        if (invoice.status !== "PENDING")
            return { statusCode: 200, text: "Payment already processed." };
        const locked = await prisma_1.prisma.paymentInvoice.updateMany({
        ...
        try {
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (fresh.type === "WALLET_TOPUP") {
                    const user = await wallet_service_1.WalletService.credit(fresh.userId, fresh.amount, `شارژ کیف پول با پرداخت آنی - فاکتور ${fresh.id}`, tx);
            ...
            await prisma_1.prisma.paymentInvoice.updateMany({ where: { id: invoice.id, status: "PAID" }, data: { status: "FAILED", verifiedAt: new Date() } });
            await prisma_1.prisma.paymentAuditLog.create({ data: { userId: invoice.userId, invoiceId: invoice.id, action: "Payment Failed", metadata: JSON.stringify({ stage: "process", error: error instanceof Error ? error.message : String(error) }) } });
            return { statusCode: 500, text: "Payment processing failed." };
        }
    }
    static async deliverProduct(tx, invoice) {
        if (!invoice.productId)
            throw new Error("محصول فاکتور مشخص نیست");
        const product = await tx.product.findFirst({ where: { id: invoice.productId, AND: [(0, visibility_1.activeProductWhere)(), { category: { is: (0, visibility_1.activeCategoryWhere)() } }] } });
        if (!product)
            throw new Error("محصول پیدا نشد");
        if (product.price !== invoice.amount)
        ...
        const reserved = await tx.productAccount.updateMany({ where: { id: account.id, AND: [(0, visibility_1.availableInventoryWhere)(product.id), (0, visibility_1.unassignedInventoryWhere)()] }, data: { status: "reserved", reservedBy: invoice.userId, reservedAt } });
        if (reserved.count !== 1)
        ...
        const sold = await tx.productAccount.updateMany({ where: { id: account.id, productId: product.id, status: "reserved", reservedBy: invoice.userId, AND: [(0, visibility_1.unassignedInventoryWhere)()] }, data: { status: "sold", soldTo: invoice.userId, soldAt, reservedBy: null, reservedAt: null } });
        if (sold.count !== 1)
